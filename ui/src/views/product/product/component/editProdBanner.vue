<template>
	<div class="system-edit-banner-container">
		<el-dialog title="产品banner" v-model="isShowDialog" width="1000px">
			<el-form :model="ruleForm" size="small" label-width="90px" :rules="rules" ref="myRefForm">
				<el-row :gutter="35">
					...
					<el-col class="mb20" :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
					  <el-form-item label="展示位置" prop="bannerModel">
					  	<el-radio-group v-model="ruleForm.bannerModel" >
					  		<el-radio v-for="(item,index) in modeList"  :label="item.value">{{item.label}}</el-radio>
					  	</el-radio-group>
					  </el-form-item>
					</el-col>
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="名称" prop="bannerName">
							<el-input v-model="ruleForm.bannerName" placeholder="请输入Banner名称" clearable></el-input>
						</el-form-item>
					</el-col>
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="banner上传">
							<img v-if="ruleForm.bannerUrl" :src="ruleForm.bannerUrl" class="avatar" width="100" height="80"/>
						...
						<el-form-item label="banner" prop="bannerUrl">
							<el-input class="zx-icon__input" v-model="ruleForm.bannerUrl" placeholder="Please input" />
						</el-form-item>
					</el-col>
				</el-row>
			</el-form>
			...
			<el-table :data="tableData.data" style="width: 100%" row-key="id" default-expand-all
				...
				<el-table-column prop="bannerName" label="名称" show-overflow-tooltip width="180"> </el-table-column>
				
				<el-table-column prop="bannerModel" label="展示位置" show-overflow-tooltip>
					<template #default="scope">
						<el-tag type="success" v-if="scope.row.bannerModel==0">首页</el-tag>
						<el-tag type="success" v-else>详情页</el-tag>
					</template>
				</el-table-column>
				...
				<el-table-column label="操作"  >
					<template #default="scope">
						...
						<el-button size="mini" type="text" @click="onModifyBanner(scope.row)">修改</el-button>
					</template>
				</el-table-column>
			</el-table>
		</el-dialog>
...
import { saveProdBanner,getProdBanner,getProdBannerList ,delProdBanner} from '/@/api/product/prod/index';
...
export default defineComponent({
	name: 'systemEditProdBanner',
	components: {
	    AttachDialog,
	},
	setup(props, ctx) {
		const { proxy } = getCurrentInstance() as any;
		const state = reactive<ProdBannerSate>({
			isShowDialog: false,
			...
			rules: {
				...
				"bannerModel": { required: true, message: '展示位置', trigger: 'blur' },
		...
		// 初始化表格数据
		const initTableData = () => {
			getProdBannerList(state.prodId,{in18:state.lang}).then(res => {
		...
		const openDialog = (row: object) => {
			...
			getProdBannerList(state.prodId,{in18:row.lang}).then(res => {
				state.tableData.data = res.data
			})
		...
		const onModifyBanner = (row: TableDataRow) => {
			getProdBanner(row.id).then(res =>{
				state.ruleForm.id = res.data.id;
				state.ruleForm.bannerModel = res.data.bannerModel;
				state.ruleForm.bannerUrl = res.data.bannerUrl;
				state.ruleForm.bannerName = res.data.bannerName;
		...
		const onSubmit = () => {
			proxy.$refs['myRefForm'].validate((valid: any) => {
				if (valid) {
					...
					saveProdBanner(state.ruleForm).then(() => {
						closeDialog(); // 关闭弹窗
						//initForm();
		  ...
		  if(isPic && isLt3M){
		    ...
		    addAttach(fd).then((response) => {
		      ...
		      if (response.code === 200) {
		           ...
		           state.ruleForm.bannerUrl= response.data.urls;
		         }else{
		...
		const onHandleLanChange = (selVal: number) => {
			...
			getProdBannerList(state.prodId,{in18:state.ruleForm.lang}).then(res => {
				state.tableData.data = res.data
			})
		...
		const onTabelRowDel = (row: TableDataRow) => {
			ElMessageBox.confirm(`此操作将永久删除banner, 是否继续?`, '提示', {
				confirmButtonText: '删除',
				cancelButtonText: '取消',
				type: 'warning',
			}).then(() => {
					delProdBanner(row.id).then((res) => {
						if(res.code == 200) {
